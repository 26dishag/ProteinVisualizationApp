...
def interactive_protein_heatmap_with_sites(protein_df, threshold, smoothing_sigma):
    ...
    all_rows = [
        'Residues', 'Peptide Residues', 'Peptide Hotspots', 'Signal Peptide/Strand', 'Predicted Score',
        'Smoothed Binary Prediction', 'Known Peptide', 'Predicted Peptides', 'Conservation',
        'Pathogenicity', 'relASA', 'Secondary Structure', 'Sites'
    ]
    n_heatmap_rows = len(all_rows)
    row_heights = [0.05, 0.05, 0.015, 0.015] + [0.05] * (n_heatmap_rows - 4)
    ...
    fig.add_trace(go.Heatmap(z=[[0] * len(positions)], x=positions, y=['Residues'], colorscale=[[0, 'white'], [1, 'white']], showscale=False, hoverinfo='skip', text=[residues_styled], texttemplate='%{text}', textfont={'size': 14}, xgap=0, ygap=0, zsmooth=False), row=1, col=1)
    fig.add_trace(go.Heatmap(z=[[0] * len(positions)], x=positions, y=['Peptide Residues'], colorscale=[[0, 'white'], [1, 'white']], showscale=False, hoverinfo='skip', text=[residues_peptide_styled], texttemplate='%{text}', textfont={'size': 14}, xgap=0, ygap=0, zsmooth=False), row=2, col=1)
    fig.add_trace(go.Heatmap(z=[underline_mask.astype(int)], x=positions, y=['Peptide Hotspots'], colorscale=[[0, 'white'], [1, '#254373']], showscale=False, hoverinfo='x+y+z', xgap=0, ygap=0, zsmooth=False), row=3, col=1)
    fig.add_trace(go.Heatmap(z=[protein_df['signal_peptide_or_Strand'].fillna(0).values], x=positions, y=['Signal Peptide/Strand'], colorscale=[[0, 'white'], [1, '#6a4c93']], showscale=False, hoverinfo='text', text=[[f"Signal: {int(val)}" for val in protein_df['signal_peptide_or_Strand'].fillna(0).values]], xgap=0, ygap=0, zsmooth=False), row=4, col=1)

    row_mapping = {
        'Predicted Score': 5,
        'Smoothed Binary Prediction': 6,
        'Known Peptide': 7,
    ...
    pastel_seq = [[0.0, '#fce4ec'], [0.2, '#f8bbd0'], [0.4, '#f48fb1'], [0.6, '#f06292'], [0.8, '#ec407a'], [1.0, '#e91e63']]
    ...
    for name, row in row_mapping.items():
        ...
        fig.add_trace(go.Heatmap(z=[data_rows[name]], x=positions, y=[name], colorscale=colorscale, showscale=False, zsmooth='fast', hoverinfo='x+y+z', xgap=0, ygap=0), row=row, col=1)

    ss_map_full = {"H": "Alpha helix (4-12)", "B": "Isolated beta-bridge residue", "E": "Strand", "G": "3-10 helix", "I": "Pi helix", "T": "Turn", "S": "Bend", "P": "Kappa helix", "-": "Unknown"}
    ...
    fig.add_trace(go.Heatmap(z=[ss_numeric], x=positions, y=['Secondary Structure'], colorscale=ss_colorscale, showscale=False, hoverinfo='text', text=[[ss_map_full.get(ss, 'Unknown') for ss in protein_df['SS'].fillna('-')]], xgap=0, ygap=0, zsmooth=False), row=12, col=1)
    ...
    fig.add_trace(go.Heatmap(z=[site_numeric], x=positions, y=['Sites'], colorscale=site_colorscale, showscale=False, hoverinfo='text', text=[[f'Site: {s}' for s in protein_df['sites']]], xgap=0, ygap=0, zsmooth=False), row=13, col=1)

    for r in range(1, n_heatmap_rows + 1):
        show_ticks = (r == n_heatmap_rows)
        fig.update_xaxes(showticklabels=show_ticks, row=r, col=1)

    fig.update_layout(autosize=True, width=None, height=int(1200 * 0.05 * n_heatmap_rows), margin=dict(t=100, b=150, r=100), font=dict(size=11), hovermode='x unified')
